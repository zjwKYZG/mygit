	...
	TableTree.prototype = {
		...
		zTreeReady : function(listData) {
			...
			function onClick(event, treeId, treeNode, clickFlag) {
				...
				$(document).scrollTop(
						tNode.offset().top - self.options.scrollTop);
			...
			function addHoverDom(treeId, treeNode) {
				...
				if (treeNode.editNameFlag
					...
					return;
				var addNode = "<span class='button tree-add' id='addBtn_"
						+ treeNode.tId + "'></span>";
				node.after(addNode);
			...
			$(document).ready(
					function() {
						$.fn.zTree.init(self.options.tree.find(".ztree"),
								setting, zNodes);
					});
		},
